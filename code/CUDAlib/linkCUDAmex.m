...
if(nargin > 0)
    ...
    linkCUDAlibMex    = @(fName) mex('-cxx','-output', [mexDir '/' fName],'-v', '-lstdc++',['-L' CUDAdirectory '/lib64/'], '-lcuda', '-lcudart', '-lnppc', '-lnpps', '-lnppial', '-lnppicc', '-lnppidei', '-lnppif', '-lnppig', '-lnppim', '-lnppist', '-lnppisu', '-lnppitc', '-lcusparse', '-lcublas','-lcurand' , '-lmwblas',[objDir '/' fName '.o']);
